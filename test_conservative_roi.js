// Test CONSERVATIVE ROI Calculator Logic
function testConservativeROI() {
  ...
  // MUCH more conservative improvements
  const newDocTime = currentDocTime * 0.7; // 30% reduction (more realistic)
  const newEfficiency = Math.min(currentEfficiency * 1.15, 95); // 15% improvement
  const newOccupancy = Math.min(currentOccupancy * 1.03, 98); // 3% improvement (very conservative)
  const newDailyRate = avgDailyRate * 1.02; // 2% increase (very conservative)
  ...
  const timeSavingsMinutesPerDay = (currentDocTime - newDocTime) * occupiedBeds;
  const timeSavingsHoursPerYear = (timeSavingsMinutesPerDay / 60) * 365;
  const staffSavings = (timeSavingsHoursPerYear / 2080) * 35000; // $35k/year per FTE
  
  const ongoingAnnualCost = implementationCost * 0.18; // 18% annual cost
  ...
  console.log("Conservative ROI Test - Medium Facility:");
  ...
  console.log(`Doc Time: ${currentDocTime}min → ${newDocTime.toFixed(0)}min (saved: ${(currentDocTime - newDocTime).toFixed(0)}min per resident)`);
  ...
  if (roi < 0) {
    ...
    console.log("⚠️  WARNING: ROI still seems high");
  } else {
